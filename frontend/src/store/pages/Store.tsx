...
const Store: React.FC<StoreProps> = ({}) => {
  ...
  useEffect(()=> {
    ...
    const fetchStore = async () => {
      try {
        const responseData = await sendRequest(`/store/${slug}`)
    ...
    return (
      ...
      <>
        ...
        {!isLoading && loadedStore && (
          ...
          <div style={{width: '100%'}}>
            <img src={loadedStore.image? `/${loadedStore.image}`: 'https://images.unsplash.com/photo-1506368144590-cf6438f1cdb5?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=934&q=80'} className={classes.image}/>
          </div>    
